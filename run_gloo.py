...
from pcode.masters.master_SPAlign import MasterSPAlign
from pcode.workers.worker_fedavg import WorkerFedAvg
from pcode.workers.worker_SPAlign import WorkerSPAlign
...
def main(rank,size,conf,port):
    ...
    elif conf.algo == "SPAlign":
        master_func = MasterSPAlign
        if conf.graph.rank != 0:
            conf.SOLVER = Namespace()
            conf.SOLVER.KL_T = conf.self_distillation_temperature
            conf.SOLVER.ALPHA = getattr(conf, 'alpha', 1.0)
        worker_func = WorkerSPAlign
